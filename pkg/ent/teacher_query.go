...
func (tq *TeacherQuery) QueryStudents() *StudentQuery {
	query := &StudentQuery{config: tq.config}
...
//		GroupBy(teacher.FieldTeacherID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
func (tq *TeacherQuery) loadStudents(ctx context.Context, query *StudentQuery, nodes []*Teacher, init func(*Teacher), assign func(*Teacher, *Student)) error {
	...
	query.Where(predicate.Student(func(s *sql.Selector) {
		s.Where(sql.InValues(teacher.StudentsColumn, fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.teacher_students
		if fk == nil {
			return fmt.Errorf(`foreign-key "teacher_students" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "teacher_students" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (tgb *TeacherGroupBy) sqlScan(ctx context.Context, v any) error {
...
func (ts *TeacherSelect) sqlScan(ctx context.Context, v any) error {
